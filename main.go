...
func logGeneral(session *discordgo.Session, channel, timestamp, daemon, message string){
	embed := discordgo.MessageEmbed{
		URL:         "",
		Type:        "rich",
		Title:       "Action",
...
func discordHandler(settings *config, timestamp, daemon, message string){
	...
	for i:= 0; i < len(settings.Channels); i++{
		go logGeneral(session, *settings.Channels[i], timestamp, daemon, message)
	}
	for i:= 0; i < len(settings.Users); i++{
		...
		go logGeneral(session, channel.ID, timestamp, daemon, message)
	}
}
